...
// +----------------------------------------------------------------------+
//
// $Id: ITX.php,v 1.6 2002/08/31 22:15:55 pajoye Exp $
//
/**
...
* @access   public
* @version  $Id: ITX.php,v 1.6 2002/08/31 22:15:55 pajoye Exp $
* @package  IT[X]
*/
class HTML_IT_ITX extends HTML_IT_IntegratedTemplate {
    ...
    */
    ...
    {
        return $this->HTML_IT_ITX($root);
    }
    function HTML_IT_ITX($root = '')
    {
    ...
    */
    function replaceBlock($block, $template, $keep_content = false)
    {
        if (!isset($this->blocklist[$block])) {
            return new HTML_HTML_IT_Error("The block '$block' does not exist in the template and thus it can't be replaced.", __FILE__, __LINE__);
        }
        if ('' == $template) {
            return new HTML_HTML_IT_Error('No block content given.', __FILE__, __LINE__);
        }
        if ($keep_content) {
    ...
    */
    function addBlock($placeholder, $blockname, $template)
    {
        ...
        if ('' == $placeholder) {
            ...
            return new HTML_HTML_IT_Error('No variable placeholder given.', __FILE__, __LINE__);

        } else if ('' == $blockname || !preg_match($this->checkblocknameRegExp, $blockname) ) {

            return new HTML_HTML_IT_Error("No or invalid blockname '$blockname' given.", __FILE__, __LINE__);
            ...
            return new HTML_HTML_IT_Error('The block already exists.', __FILE__, __LINE__);

        }

        // find out where to insert the new block
        $parents = $this->findPlaceholderBlocks($placeholder);
        if (0 == __main_cnt($parents)) {

            return new HTML_HTML_IT_Error("The variable placeholder '$placeholder' was not found in the template.", __FILE__, __LINE__);

        } else if (__main_cnt($parents) > 1 ) {

            reset($parents);
            foreach ($parents as $k => $parent){
                $msg .= "$parent, ";
            }
            $msg = __main_substr($parent, -2);

            return new HTML_HTML_IT_Error("The variable placeholder '$placeholder' must be unique, found in multiple blocks '$msg'.", __FILE__, __LINE__);
    ...
    */
    ...
    {
        if ('' == $placeholder) {
            new HTML_HTML_IT_Error('No placeholder name given.', __FILE__, __LINE__);
            return '';
        }

        if ('' != $block && !isset($this->blocklist[$block])) {
            new HTML_HTML_IT_Error("Unknown block '$block'.", __FILE__, __LINE__);
            return '';
        }
        ...
        foreach ($this->functions as $func_id => $function){
            ...
            if (isset($this->callback[$function['name']])) {
                ...
                if ('' != $this->callback[$function['name']]['object']) {
                	$tmp_callback_fct_name = $this->callback[$function['name']]['object'];
                	global $$tmp_callback_fct_name;
                    $this->variableCache['__function' . $func_id . '__'] = call_user_func(
                                                                array(
                                                                    $$tmp_callback_fct_name,
    ...
    */
    function setCallbackFunction($tplfunction, $callbackfunction, $callbackobject = '')
    {
        ...
        if ('' == $tplfunction || '' == $callbackfunction) {
            return new HTML_HTML_IT_Error("No template function ('$tplfunction') and/or no callback function ('$callback') given.", __FILE__, __LINE__);
        }
    ...
    */
    function buildFunctionlist()
    {
        ...
        while (preg_match($this->functionRegExp, $template, $regs)) {
            ...
            $pos = __main_strpos($template, $regs[0]);
            $template = __main_substr($template, $pos + __main_strlen($regs[0]));
            ...
            while ('' != $head && $args2 = $this->getValue($head, ',')) {
                $arg2 = trim($args2);
                $args[] = ('"' == $arg2[0] || "'" == $arg2[0]) ? __main_substr($arg2, 1, -1) : $arg2;
                if ($arg2 == $head) {
                    break;
                }
                $head = __main_substr($head, __main_strlen($arg2) + 1);
            }
    ...
    */
    function updateBlockvariablelist($block)
    {
        ...
        // check if any inner blocks were found
        if (isset($this->blockinner[$block]) and is_array($this->blockinner[$block]) and __main_cnt($this->blockinner[$block]) > 0) {
            // loop through inner blocks, registering the variable placeholders in each
    ...
    */
    ...
    {
        $message = sprintf('HTML_IT_ITX Warning: %s [File: %s, Line: %d]',
                                $message,
                                $file,
                                $line );
